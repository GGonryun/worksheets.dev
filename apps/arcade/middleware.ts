...
const protectedPages = [portalRoutes.account.url()];
...
export default async function middleware(req: NextRequest) {
  ...
  if (isProtectedPath && !user) {
    ...
    const redirect = new URL(portalRoutes.login.url(), req.url);
    redirect.searchParams.append('redirect', pathname);
    return NextResponse.redirect(redirect.toString());
  }

  if (user && pathname === portalRoutes.login.url()) {
    const redirect = new URL(portalRoutes.account.url(), req.url);
    return NextResponse.redirect(redirect.toString());
  }
