...
export const PrizeWheel: React.FC<PrizeWheelProps> = ({
  ...
  const onTimerTick = () => {
    ...
    if (duration < upTime) {
      ...
      const ws = segments[winner];
      if (ws) {
        if (currentSegment === ws && frames > segments.length) {
          progress = duration / upTime;
          angleDelta =
  ...
  const drawWheel = () => {
    ...
    ctx.beginPath();
    ctx.arc(centerX, centerY, 5 + size * 0.15, 0, PI2, false);
    ctx.closePath();
    ...
    ctx.fillStyle = contrastColor;
    ctx.textAlign = 'center';
    ctx.fillText(buttonText, centerX, centerY + 1);
    ctx.stroke();
  ...
  const drawNeedle = () => {
    ...
    ctx.fillStyle = contrastColor;
    ctx.beginPath();
    ctx.moveTo(centerX + (5 + size * 0.05), centerY - (5 + size * 0.2));
    ctx.lineTo(centerX - (5 + size * 0.05), centerY - (5 + size * 0.2));
