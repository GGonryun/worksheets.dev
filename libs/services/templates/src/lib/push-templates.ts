...
export class PushTemplates {
  static prizePurchased(
    ...
    return {
      ...
      }</a>. Access your prize <a href="${portalRoutes.account.inventory.url({
  ...
  static lostRaffle(
    ...
    return opts.participants.map((p) => ({
      type: 'RAFFLE',
      text: `<a href="${RAFFLE_URL(opts.id)}">Giveaway #${opts.id}</a> for a ${
        opts.item.name
      } has ended!`,
      userIds: [p.user.id],
    }));
  ...
  static wonRaffle(
    ...
    return {
      ...
      text: `Congratulations! You are the winner of <a href="${RAFFLE_URL(
        opts.raffle.id
      )}">giveaway #${opts.raffle.id}</a> for a ${
        opts.item.name
      }. Visit your <a href="${ACCOUNT_INVENTORY_URL}">your inventory</a>.`,
      userIds: [opts.user.id],
    };
  }
