...
const form: ConnectionForm<'segment'> = {
  setupTime: 5,
  instructions,
  security,
  tutorialUrl: 'https://docs.worksheets.dev/tutorial/integrations/segment',
  fields: {
    apiKey: {
      type: 'sensitive',
      title: 'API Key',
      helpUrl:
        'https://docs.worksheets.dev/tutorial/integrations/segment#api-key',
      schema: z.string(),
    },
  },
};

const translator: ConnectionContextTranslationFunction<'segment'> = ({
  apiKey,
}) => {
  console.error('TODO: implement translator for segment connection');
  return {
    apiKey: apiKey as string,
  };
};

const validator: ConnectionValidationFunction<'segment'> = async (
  connection
) => {
  if (connection.apiKey === '') {
    return { error: 'API Key is required' };
  }
  console.error('TODO: implement validator for segment connection');
