...
const form: ConnectionForm<'sendGrid'> = {
  setupTime: 5,
  instructions,
  security,
  tutorialUrl: 'https://docs.worksheets.dev/tutorial/integrations/sendGrid',
  fields: {
    apiKey: {
      type: 'sensitive',
      title: 'API Key',
      helpUrl:
        'https://docs.worksheets.dev/tutorial/integrations/sendGrid#api-key',
      schema: z.string(),
    },
  },
};

const translator: ConnectionContextTranslationFunction<'sendGrid'> = ({
  apiKey,
}) => {
  console.error('TODO: implement translator for sendGrid connection');
  return {
    apiKey: apiKey as string,
  };
};

const validator: ConnectionValidationFunction<'sendGrid'> = async (
  connection
) => {
