...
const form: ConnectionForm<'pagerDuty'> = {
  setupTime: 5,
  instructions,
  security,
  tutorialUrl: 'https://docs.worksheets.dev/tutorial/integrations/pagerDuty',
  fields: {
    token: {
      type: 'sensitive',
      title: 'API Token',
      helpUrl:
        'https://docs.worksheets.dev/tutorial/integrations/pagerDuty#api-key',
      schema: z.string(),
    },
  },
};

const translator: ConnectionContextTranslationFunction<'pagerDuty'> = ({
  token,
}) => {
  console.error('TODO: implement translator for pagerDuty connection');
  return {
    token: token as string,
  };
};

const validator: ConnectionValidationFunction<'pagerDuty'> = async (
  connection
) => {
  if (connection.token === '') {
